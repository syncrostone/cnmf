...
class CNMF(object):
  ...
  def __init__(self, k=5, gSig=[4,4], merge_thresh=0.8 , p=2, backend='single_thread', n_processes=1):
      ...
      self.options = CNMFSetParms(Y,n_processes,p=p,gSig=gSig,K=K, backend=backend)
  ...
  def fit(self, images):
    ...
    --------
    neurons : np.ndarray
    	Array of shape (y,x,n) where n is the number of neurons
  	"""
	...
	return neurons = A_or.reshape(dims[1],dims[0],A_or.shape[1])
